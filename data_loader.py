...
class DataLoader(object):
    def __init__(self, dataset, parameter, step='train'):
        ...
        self.facts_num = sum([len(self.tasks[rel]) for rel in self.tasks])
        self.rel2candidates = dataset['rel2candidates']

        self.Ground = dataset['Ground']
        self.Ground_t = dataset['Ground_t']
    ...
    def next_one(self):
        ...
        for triple in support_triples:
            ...
            while True:
                ...
                if (negative not in self.Ground_t[tuple([e1, rel]+triple[3:])]):
                    break
            support_negative_triples.append([e1, rel, negative]+triple[3:])
        ...
        for triple in query_triples:
            ...
            while True:
                ...
                if (negative not in self.Ground_t[tuple([e1, rel]+triple[3:])]):
                    break
            negative_triples.append([e1, rel, negative]+triple[3:])
    ...
    def next_one_on_eval(self):
        ...
        for triple in support_triples:
            ...
            while True:
                ...
                if (negative not in self.Ground[tuple([e1, rel]+triple[3:])]):
                    break
                else:
                    shift += 1
            support_negative_triples.append([e1, rel, negative]+triple[3:])
        ...
        negative_triples = []
        e1, rel, e2 = query_triple[:3]
        for negative in curr_cand:
            if (negative not in self.Ground[tuple([e1, rel]+query_triple[3:])]):
                negative_triples.append([e1, rel, negative]+query_triple[3:])
    ...
    def next_case_one_on_eval(self):
        if self.curr_tri_idx == self.num_tris:
            return "EOT", "EOT"

        # get current triple
        query_triple = self.eval_triples[self.curr_tri_idx]
        # while query_triple.index('[PAD]')==3:
        #     self.curr_tri_idx += 1
        #     if self.curr_tri_idx == self.num_tris:
        #         return "EOT", "EOT"
        #     query_triple = self.eval_triples[self.curr_tri_idx]
